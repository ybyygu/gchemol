// test.rs
// :PROPERTIES:
// :header-args: :tangle src/molecule/test.rs
// :END:

// [[file:~/Workspace/Programming/gchemol-rs/gchemol/core/gchemol-core.note::*test.rs][test.rs:1]]
use super::*;
...
fn test_molecule_basic() {
    ...
    mol.remove_bond_between(a1, a2)
        .expect("failed to remove bond between a1 and a2");
    ...
    let (n1, n4) = mol
        .partners(&b14)
        .expect("failed to get bond partners using bond index");
    assert_eq!(n1.index(), a1.index());
    assert_eq!(n4.index(), a4.index());
    // get partners using bond
    let (n1, n4) = real_b14
        .partners(&mol)
        .expect("failed to get bond partners using bond struct");
    ...
    // pick a single atom
    let a = mol
        .get_atom(AtomIndex::new(0))
        .expect("failed to get atom with index 0");
...
fn test_molecule_scaled_position() {
    ...
    mol.set_lattice(lat);
    let fxyzs = [
        [0.41737596, 0.75597855, 0.16098257],
        [0.41565679, 0.68546917, 0.19264617],
        [0.38461882, 0.68391421, 0.22795131],
        [0.37458942, 0.74128686, 0.24842385],
    ].to_vec();
